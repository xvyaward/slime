...
CKPT_ARGS=(
   ...
   --load /root/changhun.lee/models/qwen3-1.7b-math-sft_slime/
   --save /root/changhun.lee/models/qwen3-1.7b-math-sft_slime/
   --save-interval 2
)
...
GRPO_ARGS=(
   ...
   --opd-teacher-load /root/changhun.lee/models/Qwen3-32B_torch_dist     # Teacher model path
...
ray start --head --node-ip-address ${MASTER_ADDR} --num-gpus 8 --disable-usage-stats --dashboard-host=0.0.0.0 --dashboard-port=$PORT --num-cpus=64


ray job submit --address="http://127.0.0.1:$PORT" \
   --runtime-env-json='{
     "env_vars": {
